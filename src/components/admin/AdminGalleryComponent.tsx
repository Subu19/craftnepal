...
const AdminGalleryComponent = () => {
    ...
    const deleteImage = (season, image) => {
        setPosting(true);
        axios
            .post(import.meta.env.VITE_APP_BASE_URL + import.meta.env.VITE_APP_API + "delete/gallery/" + season + "/" + image)
            .then((res) => {
                setPosting(false);
            })
    ...
    const handleSubmit = (e) => {
        ...
        axios
            .post(import.meta.env.VITE_APP_BASE_URL + import.meta.env.VITE_APP_API + "add/gallery/" + gallery[showGallery].title, fd, {
                headers: {
            ...
            {loading ? (
                ...
                <div className="a-Containner">
                    ...
                    <div className="a-Collection">
                        {showGallery != null && Array.isArray(gallery) && gallery[showGallery] ? (
                            gallery[showGallery].photos.map((photo) => {
                                return (
                                    <div className="gPhotoBox">
                                        <PhotoProvider>
                                            <PhotoView src={import.meta.env.VITE_APP_BASE_URL + "/Gallery/" + gallery[showGallery].title + "/" + photo}>
                                                <img
                                                    className="seasonPhoto"
                                                    src={import.meta.env.VITE_APP_BASE_URL + "/Gallery/" + gallery[showGallery].title + "/" + photo}
                                                ></img>
                                            </PhotoView>
                                        </PhotoProvider>
                                        <i className="material-icons deleteIcon" onClick={() => deleteImage(gallery[showGallery].title, photo)}>
                                            delete_forever
                                        </i>
                                    </div>
                                );
                            ...
                            <div className="gPhotoBox">
                                {posting ? (
                                    ...
                                    <div className="fakephoto" onClick={() => addPhotoref.current.click()}>
                                        <i className="material-icons addPhotoIcon">add_a_photo</i>
