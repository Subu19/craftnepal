...
export const useFetchLeaderboard = (board) => {
    ...
    const getData = () => {
        axios
            .get(process.env.REACT_APP_BASE_URL + process.env.REACT_APP_API + "leaderboard/" + board)
            .then((res) => {
                setLBdata(res.data);
                setLoading(false);
            })
...
export const useFetchTop10 = () => {
    ...
    const getData = () => {
        axios
            .get(process.env.REACT_APP_BASE_URL + process.env.REACT_APP_API + "leaderboard/top")
            .then((res) => {
                setLBdata(res.data);
                setLoading(false);
            })
